...
void AugmentedEtherMac::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        switch_name = getParentModule()->getFullName();
...
//            std::cout << "----------------------------------" << endl;
    ...
    bool queue_is_empty_befor_popping = txQueue->isEmpty();
    if (!currentTxFrame && !txQueue->isEmpty())
        currentTxFrame = txQueue->popPacket();

    if (!queue_is_empty_befor_popping && (duplexMode || receiveState == RX_IDLE_STATE) && transmitState == TX_IDLE_STATE) {
