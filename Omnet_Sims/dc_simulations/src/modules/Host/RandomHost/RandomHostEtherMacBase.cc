...
#include "./RandomHostEtherMacBase.h"
...
const double RandomHostEtherMacBase::SPEED_OF_LIGHT_IN_CABLE = 200000000.0;

const RandomHostEtherMacBase::EtherDescr RandomHostEtherMacBase::nullEtherDescr = {
    0.0,
    0.0,
    0,
    B(0),
...
const RandomHostEtherMacBase::EtherDescr RandomHostEtherMacBase::etherDescrs[NUM_OF_ETHERDESCRS] = {
    {
        ETHERNET_TXRATE,
        0.5 / ETHERNET_TXRATE,
        0,
...
simsignal_t RandomHostEtherMacBase::rxPkOkSignal = registerSignal("rxPkOk");
simsignal_t RandomHostEtherMacBase::txPausePkUnitsSignal = registerSignal("txPausePkUnits");
simsignal_t RandomHostEtherMacBase::rxPausePkUnitsSignal = registerSignal("rxPausePkUnits");

simsignal_t RandomHostEtherMacBase::transmissionStateChangedSignal = registerSignal("transmissionStateChanged");
simsignal_t RandomHostEtherMacBase::receptionStateChangedSignal = registerSignal("receptionStateChanged");

RandomHostEtherMacBase::RandomHostEtherMacBase()
{
...
void RandomHostEtherMacBase::initialize(int stage)
{
    RandomHostMacProtocolBase::initialize(stage);
...
void RandomHostEtherMacBase::initializeQueue()
{
    txQueue = check_and_cast<queueing::IPacketQueue *>(getSubmodule("queue"));
}

void RandomHostEtherMacBase::initializeFlags()
{
...
void RandomHostEtherMacBase::initializeStatistics()
{
...
void RandomHostEtherMacBase::configureInterfaceEntry()
{
...
void RandomHostEtherMacBase::handleStartOperation(LifecycleOperation *operation)
{
...
void RandomHostEtherMacBase::handleStopOperation(LifecycleOperation *operation)
{
...
void RandomHostEtherMacBase::handleCrashOperation(LifecycleOperation *operation)
{
//    clearQueue();
    connected = false;
    interfaceEntry->setCarrier(false);
...
void RandomHostEtherMacBase::processAtHandleMessageFinished()
{
...
void RandomHostEtherMacBase::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
    Enter_Method_Silent();

    RandomHostMacProtocolBase::receiveSignal(source, signalID, obj, details);
...
void RandomHostEtherMacBase::processConnectDisconnect()
{
    if (!connected) {
...
void RandomHostEtherMacBase::encapsulate(Packet *frame)
{
...
void RandomHostEtherMacBase::decapsulate(Packet *packet)
{
...
bool RandomHostEtherMacBase::verifyCrcAndLength(Packet *packet)
{
    EV_STATICCONTEXT;
...
void RandomHostEtherMacBase::refreshConnection()
{
    Enter_Method_Silent();
...
bool RandomHostEtherMacBase::dropFrameNotForUs(Packet *packet, const Ptr<const EthernetMacHeader>& frame)
{
...
void RandomHostEtherMacBase::readChannelParameters(bool errorWhenAsymmetric)
{
...
void RandomHostEtherMacBase::printParameters()
{
    // Dump parameters
...
void RandomHostEtherMacBase::finish()
{
    if (!disabled) {
        simtime_t t = simTime();
...
void RandomHostEtherMacBase::refreshDisplay() const
{
    RandomHostMacProtocolBase::refreshDisplay();
...
void RandomHostEtherMacBase::changeTransmissionState(MacTransmitState newState)
{
    transmitState = newState;
    emit(transmissionStateChangedSignal, newState);
}

void RandomHostEtherMacBase::changeReceptionState(MacReceiveState newState)
{
    receiveState = newState;
    emit(receptionStateChangedSignal, newState);
}

void RandomHostEtherMacBase::addPaddingAndSetFcs(Packet *packet, B requiredMinBytes) const
{
