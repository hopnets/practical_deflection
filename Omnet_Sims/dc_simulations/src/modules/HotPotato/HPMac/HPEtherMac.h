...
#ifndef __INET_HPETHERMAC_H
#define __INET_HPETHERMAC_H
 ...
 */
class INET_API HPEtherMac : public EtherMacBase
{
  public:
    HPEtherMac() {}
    virtual ~HPEtherMac();
  ...
  protected:
    ...
    bool use_ecn;
    bool should_bounce;
    std::map<long, simtime_t> flow_packet_arrival_time;
    std::map<long, simtime_t> flow_packet_estimated_ipg;
    std::map<long, simtime_t> flow_packet_ipg_var;
    bool keep_flow_states;
    bool only_bounce_micor_burst_flows;
    int microburst_queued_packet_number_thresh;
  ...
  protected:
    ...
    bool is_packet_for_a_bursty_flow(Packet *packet);
    bool should_bounce_packet(Packet *packet);
};
