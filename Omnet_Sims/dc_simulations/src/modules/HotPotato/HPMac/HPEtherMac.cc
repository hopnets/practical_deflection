...
Define_Module(HPEtherMac);

simsignal_t HPEtherMac::collisionSignal = registerSignal("collision");
simsignal_t HPEtherMac::backoffSlotsGeneratedSignal = registerSignal("backoffSlotsGenerated");
simsignal_t HPEtherMac::feedBackPacketDroppedSignal = registerSignal("feedBackPacketDropped");
simsignal_t HPEtherMac::feedBackPacketGeneratedSignal = registerSignal("feedBackPacketGenerated");
simsignal_t HPEtherMac::feedBackPacketGeneratedReqIDSignal = registerSignal("feedBackPacketGeneratedReqID");
simsignal_t HPEtherMac::bounceLimitPassedSignal = registerSignal("bounceLimitPassed");

simsignal_t HPEtherMac::ipgSignal = registerSignal("ipg");

HPEtherMac::~HPEtherMac()
{
...
void HPEtherMac::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        use_ecn = par("use_ecn");
        should_bounce = par("should_bounce");
        keep_flow_states = par("keep_flow_states");
        only_bounce_micor_burst_flows = par("only_bounce_micor_burst_flows");
        microburst_queued_packet_number_thresh = par("microburst_queued_packet_number_thresh");
...
void HPEtherMac::initializeStatistics()
{
...
void HPEtherMac::initializeFlags()
{
...
void HPEtherMac::processConnectDisconnect()
{
    if (!connected) {
        delete frameBeingReceived;
...
void HPEtherMac::readChannelParameters(bool errorWhenAsymmetric)
{
...
void HPEtherMac::handleSelfMessage(cMessage *msg)
{
...
void HPEtherMac::handleMessageWhenUp(cMessage *msg)
{
    if (channelsDiffer)
...
bool HPEtherMac::is_packet_for_a_bursty_flow(Packet *packet) {
    bool is_packet_bursty;
...
//    is_packet_bursty = payload->getIs_micro_burst_flow();
//    delete temp;
//    delete packet_dup;

    int counter = 0;
    int old_queue_length = txQueue->getNumPackets();

    auto main_packet = packet->dup();
    main_packet->removeAtFront<EthernetMacHeader>();
    auto main_ip_header = main_packet->removeAtFront<Ipv4Header>();
    auto main_tcp_header = main_packet->removeAtFront<tcp::TcpHeader>();
    for (int i = 0; i < txQueue->getNumPackets(); i++) {
        auto queued_packet = txQueue->getPacket(i)->dup();
        queued_packet->removeAtFront<EthernetMacHeader>();
        auto queued_ip_header = queued_packet->removeAtFront<Ipv4Header>();
        auto queued_tcp_header = queued_packet->removeAtFront<tcp::TcpHeader>();

        if (main_ip_header->getSourceAddress() == queued_ip_header->getSourceAddress() &&
                main_ip_header->getDestinationAddress() == queued_ip_header->getDestinationAddress() &&
                main_tcp_header->getSourcePort() == queued_tcp_header->getSourcePort() &&
                main_tcp_header->getDestPort() == queued_tcp_header->getDestPort()) {
            counter++;
        }
    ...
    int new_queue_length = txQueue->getNumPackets();

    if (old_queue_length != new_queue_length)
        throw cRuntimeError("Just by iterating over the queue, you changed the queue length! WTH?");

    is_packet_bursty = counter <= microburst_queued_packet_number_thresh;
    EV << "With counter = " << counter << " and microburst_queued_packet_number_thresh = " << microburst_queued_packet_number_thresh << ", is packet bursty? " << is_packet_bursty << endl;

    return is_packet_bursty;
}

bool HPEtherMac::should_bounce_packet(Packet *packet) {
    auto frame = packet->peekAtFront<EthernetMacHeader>();
    std::string protocol = packet->getName();
    bool is_not_arp_related = (protocol.find("arp") == std::string::npos);
    bool is_tcp_seg = (protocol.find("tcpseg") != std::string::npos);
    bool is_queue_full = (txQueue->getMaxNumPackets() != -1 && txQueue->getNumPackets() >= txQueue->getMaxNumPackets()) ||
            (txQueue->getMaxTotalLength() != b(-1) && (txQueue->getTotalLength() + b(packet->getBitLength())) > txQueue->getMaxTotalLength());

    bool packet_allowed_to_bounce = should_bounce && (!frame->getDest().isBroadcast()) && is_not_arp_related &&  is_queue_full;
    if (packet_allowed_to_bounce && is_tcp_seg && only_bounce_micor_burst_flows) {
        packet_allowed_to_bounce = is_packet_for_a_bursty_flow(packet);
    }

    return packet_allowed_to_bounce;

}

void HPEtherMac::handleUpperPacket(Packet *packet)
{
    ...
    EV_INFO << "SEPEHR: A packet is being inserted into the queue. Queue length: "
                << txQueue->getNumPackets() << " & packetCapacity: " << txQueue->getMaxNumPackets() <<
                ", Queue data occupancy is " << txQueue->getTotalLength() <<
                "and dataCapacity is " << txQueue->getMaxTotalLength() << endl;

    if (should_bounce_packet(packet)) {
        EV << "SEPEHR: There is no place in the queue for this packet: " << packet << endl;
        auto frame = packet->removeAtFront<EthernetMacHeader>();
        if (frame->isFB()){
//            std::cout << "I am " << getParentModule()->getParentModule()->getFullName() << "." << getParentModule()->getFullName() << endl;
            EV << "Time: " << simTime() << ", Dropping packet for intra-switch congestion: " << packet->str() << endl;
//            throw cRuntimeError("Gotchaa!!!!");
            emit(feedBackPacketDroppedSignal, packet->getId());
            delete packet;
        }
        else {
            auto ip_header = packet->removeAtFront<Ipv4Header>();
            short int ttl = ip_header->getTimeToLive();
            EV << "SEPEHR: There are " << ttl << " bounces left." << endl;
            if (ttl <= 0) {
                EV << "SEPEHR: Bounce limit passed! Dropping packet" << endl;
            ...
            if (only_bounce_micor_burst_flows) {
                std::string protocol = packet->getName();
                bool is_tcp_seg = (protocol.find("tcpseg") != std::string::npos);
                if (is_tcp_seg) {
                    auto packet_dup = packet->dup();
                    ...
                    temp->insertAtBack(main_chunk);
                    auto payload = temp->popAtFront<GenericAppMsg>();
                    emit(feedBackPacketGeneratedReqIDSignal, payload->getRequesterID());
                    delete temp;
                    delete packet_dup;
                }
            }
            ...
            EV << "SEPEHR: Change src and dest address and send it back to the upper layer." << endl;
            ...
            frame->setDest(src);
            frame->setIsFB(true);
            frame->setAllow_same_input_output(true);
...
void HPEtherMac::addReceptionInReconnectState(long packetTreeId, simtime_t endRxTime)
{
...
void HPEtherMac::addReception(simtime_t endRxTime)
{
...
void HPEtherMac::processReceivedJam(EthernetJamSignal *jam)
{
...
void HPEtherMac::processJamSignalFromNetwork(EthernetSignal *msg)
{
...
void HPEtherMac::processMsgFromNetwork(EthernetSignal *signal)
{
...
void HPEtherMac::processDetectedCollision()
{
...
void HPEtherMac::handleEndIFGPeriod()
{
...
B HPEtherMac::calculateMinFrameLength()
{
...
void HPEtherMac::handleEndTxPeriod()
{
...
void HPEtherMac::scheduleEndRxPeriod(EthernetSignal *frame)
{
...
void HPEtherMac::handleEndRxPeriod()
{
    simtime_t dt = simTime() - channelBusySince;
...
void HPEtherMac::handleEndBackoffPeriod()
{
...
void HPEtherMac::handleEndJammingPeriod()
{
...
void HPEtherMac::handleRetransmission()
{
...
std::list<int> HPEtherMac::getInfoFlowByteArray(std::string srcAddr, std::string destAddr, int srcPort, int destPort) {
    std::list<int> bytes;
    std::string token;
...
unsigned int HPEtherMac::Compute_CRC16_Simple(std::list<int> bytes, int bytes_size){
...
void HPEtherMac::frameReceptionComplete()
{
    ...
    //TODO HERE IS FOR CALCULATING INTER-PACKET-GAP. WE MIGHT NEED IT IN THE FUTURE
    std::string protocol = packet->getName();
//    bool is_not_arp_related = (protocol.find("arp") == std::string::npos);
    bool is_tcp_seg = (protocol.find("tcpseg") != std::string::npos);
    if (keep_flow_states && is_tcp_seg && getParentModule()->getIndex() == 1) {
        auto packet_dup = packet->dup();
        packet_dup->removeAtFront<EthernetMacHeader>();
        auto ip_header = packet_dup->removeAtFront<Ipv4Header>();
        auto tcp_header = packet_dup->removeAtFront<tcp::TcpHeader>();
        std::list<int> bytes = getInfoFlowByteArray(ip_header->getSourceAddress().str(), ip_header->getDestinationAddress().str(), tcp_header->getSourcePort(), tcp_header->getDestinationPort());
        int bytes_size = bytes.size();
        unsigned int crc = Compute_CRC16_Simple(bytes, bytes_size);
        delete packet_dup;
        if (flow_packet_arrival_time.count(crc) == 0)
            flow_packet_arrival_time[crc] = simTime();
        else {
            simtime_t now = simTime();
            simtime_t ipg = now - flow_packet_arrival_time[crc];
            std::cout << "time: " << simTime() << ", ipg is " << ipg << endl;
            if (flow_packet_estimated_ipg.count(crc) == 0) {
                flow_packet_estimated_ipg[crc] = ipg;
                flow_packet_ipg_var[crc] = ipg / 2;
            } else {
                if (ipg >= flow_packet_estimated_ipg[crc])
                    flow_packet_ipg_var[crc] = (3 * 0.25 * flow_packet_ipg_var[crc]) + (0.25 * (ipg - flow_packet_estimated_ipg[crc]));
                else
                    flow_packet_ipg_var[crc] = (3 * 0.25 * flow_packet_ipg_var[crc]) + (0.25 * (flow_packet_estimated_ipg[crc] - ipg));
                flow_packet_estimated_ipg[crc] = (7 * 0.125 * flow_packet_estimated_ipg[crc]) + (0.125 * ipg);
            }
            flow_packet_arrival_time[crc] = now;
            std::cout << "estimated ipg is: " << flow_packet_estimated_ipg[crc] << " and ipg var is: " <<
                    flow_packet_ipg_var[crc] << endl;
            emit(ipgSignal, flow_packet_ipg_var[crc]);
        }
    }
...
//            throw cRuntimeError("Gotchaa2!!!!");
            ...
            EV << "SEPEHR: This packet is NOT a feedback packet: " << packet << endl;
        }
        processReceivedDataFrame(packet);
    }
}

void HPEtherMac::processReceivedDataFrame(Packet *packet)
{
    // statistics
...
void HPEtherMac::processReceivedControlFrame(Packet *packet)
{
...
void HPEtherMac::scheduleEndIFGPeriod()
{
...
void HPEtherMac::fillIFGIfInBurst()
{
    if (!frameBursting)
        return;
...
void HPEtherMac::scheduleEndTxPeriod(B sentFrameByteLength)
{
...
void HPEtherMac::scheduleEndPausePeriod(int pauseUnits)
{
...
void HPEtherMac::beginSendFrames()
{
    if (currentTxFrame) {
