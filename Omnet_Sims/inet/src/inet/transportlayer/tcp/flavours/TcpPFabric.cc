...
#include "inet/transportlayer/tcp/flavours/TcpPFabric.h"

namespace inet {
namespace tcp {

Register_Class(TcpPFabric);

#define PFABRIC_CONSTANT_RTO 0.0006
#define PFABRIC_CONSTANT_REXMIT_COUNT 7 // anything less than max rexmit count

TcpPFabric::TcpPFabric() : TcpTahoeRenoFamily(),
    state((TcpRenoStateVariables *&)TcpAlgorithm::state)
{
}

void TcpPFabric::initialize()
{

    TcpBaseAlg::initialize();
    if (!state->FRs_disabled)
        throw cRuntimeError("FR should be disabled for pFabric!");
    state->in_probe_mode = false;
}

void TcpPFabric::recalculateSlowStartThreshold()
{
    // RFC 2581, page 4:
...
void TcpPFabric::processRexmitTimer(TcpEventCode& event)
{
    TcpTahoeRenoFamily::processRexmitTimer(event);

    if (state->rexmit_timeout != PFABRIC_CONSTANT_RTO)
        throw cRuntimeError("state->rexmit_timeout != PFABRIC_CONSTANT_RTO");

    EV << "TcpPFabric::processRexmitTimer: rexmit_count is " << state->rexmit_count << endl;
    ...
    if (state->rexmit_count >= 5) {
        // probe mode
        EV << "Entered probe mode, setting cwnd to 1B" << endl;
        state->snd_cwnd = 1;
        state->in_probe_mode = true;
        state->rexmit_count = std::min(state->rexmit_count, PFABRIC_CONSTANT_REXMIT_COUNT);
    } else {
...
void TcpPFabric::receivedDataAck(uint32 firstSeqAcked)
{

//    if (!state->sack_enabled)
//        throw cRuntimeError("sack should be enabled for pFabric");

    EV << "This is TcpPFabric talking!" << endl;
...
//                conn->emit(ssthreshSignal, state->ssthresh);
        ...
        if (performSsCa) {
            ...
            if (state->in_probe_mode) {
                EV << "Exited probe mode. Restarting slow start." << endl;
                state->in_probe_mode = false;
                state->snd_cwnd = 0; // this is increased to 1 mss by slow start
            }
            ...
            if (state->snd_cwnd < state->ssthresh) {
                ...
                if (state->snd_cwnd == 0)
                    throw cRuntimeError("state->snd_cwnd == 0 here is wrong!");
...
void TcpPFabric::receivedDuplicateAck()
{
    if (!state->FRs_disabled) {
